 ...
 * [The Big List of Naughty Strings](https://github.com/minimaxir/big-list-of-naughty-strings) for
 * various test-related inputs.
 */
@Suppress("unused")
class Blns @JvmOverloads constructor(config: FakerConfig = fakerConfig {}) : AbstractFaker(config) {
    ...
    fun get(vararg category: Category): Map<Category, List<String>> {
        val inStr =
            requireNotNull(javaClass.classLoader.getResourceAsStream("blns_categories.json"))
    ...
    /**
     * Returns a portion of [all] strings (or [allBase64] strings if [base64] is `true`) with
     * pseudo-randomly generated `fromIndex` and (possibly) `toIndex` values.
     *
    ...
    fun sublist(size: Int, base64: Boolean = false): List<String> =
        if (base64) randomService.randomSublist(allBase64, size)
        else randomService.randomSublist(all, size)
     ...
     *   list is the result of calling [RandomService.nextInt] with the given [sizeRange]. IF `size
     *   <= 0` then `toIndex` will also be randomly-generated.
     */
    fun sublist(sizeRange: IntRange, base64: Boolean = false): List<String> =
        if (base64) randomService.randomSublist(allBase64, sizeRange)
        else randomService.randomSublist(all, sizeRange)
...
/**
 * Applies the [block] function to [Blns.Builder] and returns as an instance of [Blns] from that
 * builder.
 */
@Suppress("unused")
