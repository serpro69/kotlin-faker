 ...
 *
 * @constructor creates an instance of this [FakerService] with the default 'en' locale if is not
 *   specified.
 */
class FakerService {
    ...
    private fun getAllLocaleOrCategoryFileStreams(
        ...
        val resourceFilenames =
            listOfNotNull(
                ...
                .distinct()
        val localeStreams =
            resourceFilenames.mapNotNull { javaClass.classLoader.getResourceAsStream(it) }
        require(localeStreams.isNotEmpty() || locale in listOf("fr", "ja")) {
    ...
    private fun getLocaleFileStream(locale: String): InputStream? {
        return javaClass.classLoader.getResourceAsStream("locales/$locale.json")
            ?: javaClass.classLoader.getResourceAsStream(
                "locales/${locale.substringBefore("-")}.json"
            )
    }

    /**
     * Merges [default] and [localized] categories (providers) and values, using localized value if
     * present.
     *
     * Will also handle partially-localized categories, including partially-localized functions with
     * secondary_keys, for example:
     *
     * IF (en.category.function.secondary_key1 AND en.category.function.secondary_key2) IS PRESENT
     * AND <locale>.category.function.secondary_key1 IS PRESENT AND
     * en.category.function.another_secondary_key2 IS ABSENT THEN RETURN
     * <locale>.category.function.secondary_key1 AND en.category.function.secondary_key2
     *
    ...
    private fun merge(
        ...
        localized.forEach { (key, localizedMap) ->
            default[key]?.let { enMap ->
                 ...
                 */
                default[key] =
                    localizedMap.mapValuesTo(linkedMapOf()) { (k, v) ->
                        /*
                        ...
                        if (v is Map<*, *> && enMap.containsKey(k)) {
                            // check if function has a secondary_key that is used to resolve the
                            // values
                            // if true we assume that u[k] should also be a Map because the
                            // structure of dict files
                            // should match
                            // v IS en.faker.games.<secondary_key> (i.e pokemon)
                            (enMap[k] as Map<*, *>).plus(v)
                        } else if (enMap.containsKey(k)) {
                            // check if the primary_key (function_name) matches with localized
                            // provider
                            // if v is not a map, but localized key matches, then use the values for
                            // that key
                            localizedMap[k]
                        } else {
    ...
    private fun computePhoneNumber(category: YamlCategory, locale: String): HashMap<String, Any>? {
        val instr =
            when (locale) {
                "en",
                "ja",
                "fr" -> getCategoryFileStream(locale, PHONE_NUMBER, null)
                else -> getLocaleFileStream(locale)
            }
        return instr?.use {
            when (category) {
                PHONE_NUMBER,
                CELL_PHONE -> readCategoryOrNull(it, locale, category) as HashMap?
                else -> null
            }
        }
    }
    ...
    private fun computeSymbol(category: YamlCategory, locale: String): Any? {
        val localeData =
            getLocaleFileStream(locale)?.use {
     ...
     *
     * Additionally, `if (locale != null && locale.isValid)`, reads the contents of the specified
     * locale file into this [dictionary] (Will overwrite all matching keys with values from
     * specified locale file.)
     *
     * @throws IllegalArgumentException if the [locale] is invalid or locale dictionary file is not
            ...
            when (category) {
                ...
                CURRENCY_SYMBOL -> {
                    ...
                    if (locale != "en") { // 'en' is already processed at this point
                        val inputs =
                            getAllLocaleOrCategoryFileStreams(locale, category, secondaryCategory)
                        ...
                        inputs.forEach { input ->
                            input.use { instr ->
                                readCategoryOrNull(instr, locale, category)?.let {
                                    ...
                                    localized?.forEach { m ->
                                        defaultValues.merge(m.key, m.value as Any) { orig, loc ->
                                            loc
                                        }
                                    }
                                }
     ...
     * where each `key` represents a function in the data category, i.e. `address.country`, and
     * `value` represents values (of unknown type) from this function.
     *
    ...
    private fun readCategoryOrNull(
        ...
        val localeData: Map<*, *> =
            with(Mapper.readValue(inputStream, Map::class.java)) {
     ...
     *
     * @throws DictionaryKeyNotFoundException IF the [dictionary] [category] does not contain the
     *   [key]
     */
    fun getRawValue(category: YamlCategory, key: String): RawExpression {
        val paramValue =
            getProviderData(category)[key]
                ?: throw DictionaryKeyNotFoundException(
                    "Parameter '$key' not found in '$category' category"
                )

        return when (paramValue) {
            is List<*> -> {
                if (paramValue.isEmpty()) RawExpression("")
                else
                    when (val value = randomService.randomValue(paramValue)) {
                        is List<*> -> {
                            if (value.isEmpty()) RawExpression("")
                            else RawExpression(randomService.randomValue(value) as String)
                        }
                        is String -> RawExpression(value)
                        is Int -> RawExpression(value.toString())
                        else ->
                            throw UnsupportedOperationException(
                                "Unsupported type of raw value: ${paramValue::class.simpleName}"
                            )
                    }
            }
    ...
    /**
     * Returns raw value as [RawExpression] from a given [category] fetched by its [key] and
     * [secondaryKey]
     *
     * @throws DictionaryKeyNotFoundException IF the [dictionary] [category] does not contain the
     *   [key], OR the primary [key] does not contain the [secondaryKey]
     */
            ...
            is Map<*, *> -> {
                if (secondaryKey == "") {
                    ...
                    when (val secondaryValue = randomService.randomValue(mapValues)) {
                        is List<*> ->
                            RawExpression(randomService.randomValue(secondaryValue) as String)
                        is String -> RawExpression(secondaryValue)
                        is Map<*, *> -> RawExpression(secondaryValue.toString())
                        else ->
                            throw UnsupportedOperationException(
                                "Unsupported type of raw value: ${parameterValue::class.simpleName}"
                            )
                    }
                } else {
                    parameterValue[secondaryKey]?.let { secondaryValue ->
                        ...
                        ?: throw DictionaryKeyNotFoundException(
                            "Secondary key '$secondaryKey' not found."
                        )
                }
            }
            else ->
    ...
    /**
     * Returns raw value as [RawExpression] for a given [category] fetched from the [dictionary] by
     * its [key], [secondaryKey], and [thirdKey].
     *
     * @throws DictionaryKeyNotFoundException IF the [dictionary] [category] does not contain the
     *   [key], OR the primary [key] does not contain the [secondaryKey], OR the [secondaryKey] does
     *   not contain the [thirdKey]
     */
    fun getRawValue(
        category: YamlCategory,
        key: String,
                            ...
                            is Map<*, *> -> {
                                if (thirdKey == "") {
                                    ...
                                    when (val thirdValue = randomService.randomValue(mapValues)) {
                                        is List<*> ->
                                            RawExpression(
                                                randomService.randomValue(thirdValue) as String
                                            )
    ...
    /**
     * Resolves [RawExpression] value of the [key], [secondaryKey], and [thirdKey] in this
     * [category].
     */
    fun resolve(
        category: YamlCategory,
     ...
     * - `#{city_prefix}` from `en: faker: address` would be resolved to getting value from
     *   `address: city_prefix`
     * - `#{Name.first_name} from `en: faker: address` would be resolved to calling [Name.name]
     *   function.
     * - `#{Educator.tertiary.degree.type}` from `en: faker: educator: degree` would be resolved to
     *   calling [Degree.type] function. In this case the chained call needs to be implemented as a
     *   "class->property" hierarchy, i.e. [Educator] class must declare a `tertiary` property of
     *   [Tertiary] type, which in turn must declare a `degree` property of [Degree] type, and so
     *   on.
     *
     * Recursive expressions are also supported:
     * - `#{Name.name}` from `en: faker: book: author` that could be resolved to
     *   `#{first_name} #{last_name}` from `en: faker: name: name` will be resolved to concatenating
     *   values from `en: faker: name: first_name` and `en: faker: name: last_name` and so on until
     *   the expression is exhausted to the actual value.
     *
     * *It is worth noting that `'#'` and `'?'` chars will not be numerified/letterified inside this
     * function, and will be returned as is. To further numerify/letterify the resolved expression,
     * one must do that explicitly where it is needed.*
     *
     * For example, if the resolved string `"???###"` needs to be further numerified/letterified to
     * return `XYZ012`, where `XYZ` is a combination of pseudo-randomly generated English letters in
     * upper-case, and `012` is a combination of pseudo-randomly generated digits, it could be done
     * like so:
     * ```
     ...
     * but can be omitted for calls to other [category]s. For example:
     *
     * `address.yml`:
     * ```
     * en:
    ...
    private tailrec fun resolveExpression(
        category: YamlCategory,
        ...
        val cexpr =
            Regex("""#\{(?!\d)(?!(?i:$primary|(?i:$secondary))\.)(?:([A-Z]\p{L}+())\.)?(.*?)\}""")
        val sb = StringBuffer()
        val pc: (Matcher) -> YamlCategory? = {
            it.group(1)?.let { c -> YamlCategory.findByName(c) }
        }
        val sc: (Matcher) -> Category? = {
            it.group(2)?.let { c -> Category.ofName(c.uppercase()) }
        }
            ...
            when {
                lexpr.containsMatchIn(rawExpression.value) -> {
                    findMatchesAndAppendTail(rawExpression.value, sb, lexpr) {
                        val args =
                            sc(it)?.let { c ->
                                "${c.name.lowercase()}.${it.group(3)}".split(".").toTypedArray()
                ...
                if (lexpr.containsMatchIn(resolvedExpression)) {
                    ...
                    when { // resolve expression from another category, rinse and repeat
                        cm.find() -> {
                            ...
                            resolveExpression(
                                cat,
                                RawExpression(resolvedExpression.replace(cc, "")),
                            )
                        }
    ...
    val String.numerify: () -> String
        get() = {
            map { if (it == '#') randomService.nextInt(10).toString() else "$it" }.joinToString("")
        }

    /**
     * Replaces every `?` char for this [String] receiver with a random letter from the English
    ...
    val String.letterify: (upper: Boolean?) -> String
        get() = { upper ->
            map {
                    if (it == '?') {
                        randomService
                            .nextLetter(upper = upper ?: randomService.nextBoolean())
                            .toString()
                    } else "$it"
                }
                .joinToString("")
    ...
    /**
     * Replaces every `?` char for this [String] receiver with a random upper-case letter from the
     * English alphabet and returns the modified [String].
     */
    ...
    /**
     * Returns an instance of [FakeDataProvider] fetched by its [simpleClassName]
     * (case-insensitive).
     *
