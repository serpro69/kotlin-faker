    ...
    DescribeSpec({
        ...
        describe("a TestClass with non-empty constructor") {
            ...
            context("creating a random instance of the class") {
                ...
                it(
                    "should follow precedence rules and try to generate a 'default instance' before using 'predefined instance'"
                ) {
                    class Bar(val id: Int)
                    class Baz(val bar: Bar)

                    val bar =
                        randomProvider.randomClassInstance<Bar> { typeGenerator<Bar> { Bar(0) } }

                    val baz =
                        randomProvider.randomClassInstance<Baz> {
        ...
        describe(
            "a TestClass with non-empty constructor with primitive type and custom-type parameters"
        ) {
        ...
        describe("internal constructor") {
            ...
            it("should use a non-default constructor") {
                val t =
                    randomProvider.randomClassInstance<TestClassInternal>() {
            ...
            context("creating a random instance of a class") {
                ...
                it("exception is thrown") {
                    ...
                    exception.message shouldBe
                        "No suitable constructor or predefined instance found for ${TestClassNoPublic::class}"
                }
            }
            context("creating a random instance of an interface") {
                it("should return a predefined interface instance via typeGenerator") {
                    val testInterface =
                        randomProvider.randomClassInstance<TestInterface> {
        ...
        describe("a TestClass with non-empty constructor for custom type generators") {
            ...
            context("creating a random instance of the class with custom generators") {
                ...
                it("it should be a predefined UUID and primitives") {
                    ...
                    testClass.bar shouldBe
                        "ParameterInfo(index=0, name=bar, isOptional=false, isVararg=false, type=kotlin.String, kind=VALUE)"
                }
            }
        }
        ...
        describe("a TestClass with non-empty constructor for custom parameter generators") {
            ...
            context(
                "creating a random instance of the class with custom parameter and type generators"
            ) {
                val testClass: TestClass =
                    randomProvider.randomClassInstance {
                ...
                it(
                    "should use predefined UUIDs with parameter generators taking precedence over type generators"
                ) {
                    testClass shouldBe instanceOf(TestClass::class)
                    ...
                    testClass.foo shouldBe
                        "ParameterInfo(index=4, name=foo, isOptional=true, isVararg=false, type=kotlin.String?, kind=VALUE)"
                }
            }
        }
            ...
            context("creating a random instance of the class with nullable custom generators") {
                val testClass: TestClass =
                    randomProvider.randomClassInstance {
                        ...
                        typeGenerator<Int> {
                            givenInt
                        } // use the not-null type generator and verify it is used for Int?
        ...
        describe("a TestClass with 3 non-default constructors") {
            ...
            context("creating a random instance of the class with constructor args size") {
                val testClass: TestClass =
                    randomProvider.randomClassInstance { constructorParamSize = 3 }
            ...
            context(
                "creating a random instance of the class with invalid number of args and a fallback strategy"
            ) {
                val testClass: TestClass =
                    randomProvider.randomClassInstance {
            ...
            context(
                "creating a random instance of the class with invalid number of args and a filter strategy"
            ) {
                val testClass: TestClass =
                    randomProvider.randomClassInstance {
                        constructorParamSize = 6
        ...
        describe("a TestClass with non-empty constructor with Collection-type parameters") {
            ...
            it("should generate Collections with pre-configured size") {
                val testClass =
                    randomProvider.randomClassInstance<TestClass> { collectionsSize = 10 }
                ...
                // boolean-key based map can only have 1 or 2 entries, depending on the randomness
                ...
                // enum-key based map can only have up to 3 entries in this case, depending on the
                // randomness of the
                // generated key
            ...
            it("typeGenerator should have precedence over collection and map generators") {
                val testClass =
                    randomProvider.randomClassInstance<TestClass> {
                        ...
                        typeGenerator<Map<TestEnum, TestEnum>> {
                            mapOf(TestEnum.JAVA to TestEnum.KOTLIN)
                        }
                    }
            ...
            it("should generate null elements in collections") {
                ...
                val nullable =
                    randomProvider.randomClassInstance<Nullable> {
                        ...
                        collectionElementTypeGenerator<Long?> {
                            if (!random.nextBoolean()) 0L else null
                        }
                        mapEntryKeyTypeGenerator<Char> {
                            listOf('a', 'b', 'c', 'd', 'e', 'f').random()
                        }
                        mapEntryValueTypeGenerator<String?> {
                            if (random.nextBoolean()) null else "foo"
                        }
                    }
                ...
                // but since nulls are never returned as value, all nulls will be returned as random
                // instance,
        ...
        describe("a TestClass with with abstract type constructor parameter") {
            ...
            class TestClass(val foo: Foo, val bar: Bar, val baz: Baz, val number: Number)
            ...
            it("should custom typeGenerator for Number") {
                val testClass =
                    randomProvider.randomClassInstance<TestClass> { typeGenerator<Number> { 42 } }
        ...
        describe("a primitive type") {
            it("should not throw an exception when no suitable constructor exists") {
                shouldNotThrow<NoSuchElementException> {
                    randomProvider.randomClassInstance<Double>()
                }
                shouldNotThrow<NoSuchElementException> {
                    randomProvider.randomClassInstance<Float>()
                }
                shouldNotThrow<NoSuchElementException> {
                    randomProvider.randomClassInstance<Long>()
                }
                shouldNotThrow<NoSuchElementException> { randomProvider.randomClassInstance<Int>() }
                shouldNotThrow<NoSuchElementException> {
                    randomProvider.randomClassInstance<Short>()
                }
                shouldNotThrow<NoSuchElementException> {
                    randomProvider.randomClassInstance<Byte>()
                }
                shouldNotThrow<NoSuchElementException> {
                    randomProvider.randomClassInstance<Boolean>()
                }
                shouldNotThrow<NoSuchElementException> {
                    randomProvider.randomClassInstance<Char>()
                }
            }
            ...
            context("precedence") {
                ...
                it("should use own configuration on function level") {
                    with(RandomClassProvider(cfg())) {
                        ...
                        randomClassInstance<Baz> { typeGenerator { Foo(36) } }
                            .also {
        ...
        context("copy of RandomClassProvider") {
            ...
            it(
                "should have same configuration as defined on fakerConfig level if not defined on RandomClassProvider level"
            ) {
                val c = cfg()
                with(RandomClassProvider(c)) {
                    val copy = copy()
                    configProperties(copy.config) shouldBe
                        configProperties(c.randomProviderConfig!!)
                    copy.config shouldNotBeSameInstanceAs c.randomProviderConfig
                    copy.config shouldNotBeSameInstanceAs this@with.config
                }
            }
        ...
        context("randomClassProvider top-level functions") {
            ...
            it("should configure all random instances via configurator") {
                val bar =
                    randomClassInstance<Bar> {
                        typeGenerator { Foo(42) }
                        nullableTypeGenerator { "non-null" }
                        namedParameterGenerator("test") { 32167 }
                    }
                val baz =
                    randomClassInstance<Baz> {
                        typeGenerator { Foo(42) }
                        nullableTypeGenerator { "non-null" }
                        namedParameterGenerator("test") { 32167 }
                    }
