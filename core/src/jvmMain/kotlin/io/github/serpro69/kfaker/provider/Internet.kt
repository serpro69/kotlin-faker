...
@Suppress("unused")
class Internet internal constructor(fakerService: FakerService, private val nameProvider: Name) :
        ...
        return domain?.let {
            domain
                ...
                .also {
                    ...
                    if (subdomain && it.size < 3)
                        it.add(0, prepare(name(), fakerService.faker.config))
                }
                .joinToString(".")
        }
            ?: run {
    ...
    fun iPv4Address() = List(4) { fakerService.randomService.nextInt(0, 255) }.joinToString(".")
    ...
    fun publicIPv4Address(): String =
        with(iPv4Address()) { if (isReservedNet(this)) publicIPv4Address() else this }
    ...
    fun iPv6Address(): String =
        List(8) { fakerService.randomService.nextInt(65_536).toString(16) }.joinToString(":")
    ...
    fun email(name: String = ""): String {
        val localName =
            if (name.trim() == "") {
                nameProvider.name().replace(".", "").replace(" ", ".").lowercase()
            } else name.replace(" ", "")

        return "$localName@${domain()}"
    }

    @JvmOverloads fun safeEmail(name: String = "") = "${email(name).substringBeforeLast(".")}.test"
...
// Private, Host, and Link-Local network address blocks as defined in
// https://en.wikipedia.org/wiki/IPv4#Special-use_addresses
private val privateIpv4Ranges =
    listOf(
        // 10.0.0.0/8     - Used for local communications within a private network
        listOf(10..10, 0..255, 0..255, 1..255),
        // 100.64.0.0/10  - Shared address space for communications between an ISP and its
        // subscribers
        ...
        // 198.18.0.0/15  - Used for benchmark testing of inter-network communications between
        // subnets
