...
/**
 * [FakeDataProvider] implementation class for functionality not covered by the standard dictionary
 * files.
 */
@Suppress("unused")
class Person internal constructor(private val random: Random) {
     ...
     *
     * If the [at] date parameter is not provided, then [LocalDate.now] is used to calculate the
     * "lower bound" for the resulting birthDate. Assuming the [age] value of `30`, the resulting
     * date will be in the range of:
     * ```
     ...
     * @param at a date at which the person was of the specified [age]. Equals to the result of
    ...
    fun birthDate(age: Long, at: LocalDate = LocalDate.now()): LocalDate {
        ...
        val lower =
            LocalDate.ofYearDay(startDate.year, (startDate.dayOfYear..startMaxDays).random(random))
        val upper = LocalDate.ofYearDay(endDate.year, (1..endDate.dayOfYear).random(random))
        val dates =
            if (lower.month == at.month && lower.dayOfMonth == at.dayOfMonth) {
