...
class UniqueDataProviderIT :
    DescribeSpec({
        ...
        describe(
            "collection of values is used to exclude values from being generated for specific provider"
        ) {
            val config = fakerConfig { uniqueGeneratorRetryLimit = 100 }
            ...
            repeat(10) {
                ...
                context("collection of unique values is generated run#$it") {
                    ...
                    faker.unique.configuration {
                        excludeFromFunction(Address::country, excludedCountries)
                    }
                    val newCountries = (0..30).map { faker.address.country() }

                    val moreExcludedCountries =
                        listOf(
                            "Cambodia",
                            "Cameroon",
                            "Canada",
                    ...
                    faker.unique.configuration {
                        excludeFromProvider<Address>(moreExcludedCountries)
                    }
                    val moreCountries = (0..30).map { faker.address.country() }

                    val excludedCountryCodes =
                        listOf("AD", "AE", "AF", "AG", "AI", "AL", "AM", "AO", "AQ")
                    faker.unique.configuration {
                        excludeFromProvider<Address>(excludedCountryCodes)
                    }
                    ...
                    it(
                        "already generated values through config should not be included in the generation"
                    ) {
                        assertSoftly {
                            newCountries shouldNotContainAnyOf countries
        ...
        describe(
            "use regex patterns to exclude values from being generated for specific provider"
        ) {
            repeat(10) {
                val faker = Faker()
                ...
                it(
                    "excluded values by pattern should not be included in the generation for the provider run#$it"
                ) {
                    val countries = (0..30).map { faker.address.country() }
        ...
        describe("unique generation of values for category") {
            ...
            context("collection of values is generated") {
                ...
                it("should not contain duplicates") { countries should beUnique() }
            }

            context("used values are cleared") {
                val faker = Faker(config).also { it.unique.configuration { enable(it::address) } }
                val countries = (0..20).map { faker.address.country() }
            ...
            context("unique generation is not enabled for category") {
                ...
                context("some values were marked for exclusion") {
                    ...
                    it("collection can have duplicates") { countries should containDuplicates() }
            ...
            context(
                "values are generated for another category that is not marked for unique generation"
            ) {
                val faker = Faker(config)
                faker.unique.configuration { enable(faker::address) }
                ...
                it("collection can have duplicates") { colors shouldNot beUnique() }
            }
            ...
            context("unique generation for category is disabled") {
                ...
                context("collection of values is generated") {
                    ...
                    it("collection can have duplicates") { countries shouldNot beUnique() }
                }
            }
        ...
        describe("local unique generation") {
            ...
            context("unique property prefixes the category function invocation") {
                val countries = (0..20).map { faker.address.unique.country() }

                it("collection should not contain duplicates") { countries should beUnique() }
