 ...
 *
 * Each category (generator) from this [EduFaker] is represented by a property that (usually) has
 * the same name as the `.yml` dictionary file.
 *
 * @property unique global provider for generation of unique values.
 */
@Suppress("unused")
class EduFaker @JvmOverloads constructor(config: FakerConfig = fakerConfig {}) :
...
/**
 * Applies the [block] function to [EduFaker.Builder] and returns as an instance of [EduFaker] from
 * that builder.
 */
