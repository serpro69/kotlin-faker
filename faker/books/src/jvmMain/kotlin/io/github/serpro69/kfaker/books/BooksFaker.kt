 ...
 *
 * Each category (generator) from this [BooksFaker] is represented by a property that (usually) has
 * the same name as the `.yml` dictionary file.
 *
 * @property unique global provider for generation of unique values.
 */
@Suppress("unused")
class BooksFaker @JvmOverloads constructor(config: FakerConfig = fakerConfig {}) :
    ...
    val theKingkillerChronicle: TheKingkillerChronicle by lazy {
        TheKingkillerChronicle(fakerService)
    }
...
/**
 * Applies the [block] function to [BooksFaker.Builder] and returns as an instance of [BooksFaker]
 * from that builder.
 */
fun faker(block: BooksFaker.Builder.() -> Unit): BooksFaker =
    BooksFaker.Builder().apply(block).build()
