...
class StarWarsTest :
    DescribeSpec({
        describe("StarWars provider") {
            ...
            // FakerService?
            //  StarWars(FakerService(MoviesFaker()) won't work because the constructor in
            // FakerService is internal,
            //  and we should try not to expose it to public
            //  We could expose FakerService via custom AbstractFaker implementation (e.g. see
            // edu/FakerServiceTest)
            val starWars = MoviesFaker().starWars

            context("any quote") {
                it("should return any quote") { starWars.quote() shouldNotBe null }
            }
