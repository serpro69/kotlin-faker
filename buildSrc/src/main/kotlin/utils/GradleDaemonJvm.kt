 ...
 *
 * In order to maximise Build Cache hits the same JDK version should be used to run Gradle on all
 * machines.
 *
 * Gradle has an incubating feature to set the Daemon JVM
 * https://docs.gradle.org/current/userguide/gradle_daemon.html#sec:daemon_jvm_criteria, but it
 * doesn't auto-download the required JVM. This is annoying, because Gradle will just fail until the
 * user manually installs the required JVM.
 *
 * Instead, this function configures Gradle so it will only set the Daemon JVM if the required JDK
 * is installed.
 *
...
fun configureGradleDaemonJvm(
    ...
    // Depend on some common tasks, so the Daemon JVM properties file will be generated
    // automatically.
    project.tasks
        .matching {
            it.name in setOf("prepareKotlinIdeaImport", "prepareKotlinBuildScriptModel", "assemble")
        }
        .configureEach { dependsOn(updateDaemonJvm) }
}

/**
 * Determine if the [JavaToolchainService] can detect, or automatically install, a toolchain for
 * this [JavaVersion].
 */
private fun Provider<JavaLanguageVersion>.isInstalled(
    ...
    return map { version ->
        runCatching { javaToolchains.launcherFor { languageVersion = version }.orNull }
            .getOrNull() != null
    }
}
